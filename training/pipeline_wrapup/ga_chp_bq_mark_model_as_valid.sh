...
SCORES_FILE=${MODELS_DIR}/${MODEL_DAY_AS_STR}_${UNIQUE_HASH}_ga_chp_bq_churn_scores.json
ACCURACY=$(cat ${SCORES_FILE} | jq '.accuracy')
LOSS=$(cat ${SCORES_FILE} | jq '.loss')

# Insert model stats into the Cassandra database
sed "s/MODEL_DAY_AS_STR/${MODEL_DAY_AS_STR}/;s/UNIQUE_HASH/${UNIQUE_HASH}/;s/ACCURACY/${ACCURACY}/;s/LOSS/${LOSS}/;s/THRESHOLD/${THRESHOLD}/;s/IS_MODEL_VALID/${IS_MODEL_VALID}/" /opt/ga_chp_bq/training/pipeline_wrapup/insert_into_ga_chp_bq_valid_models.cql.template > /tmp/ga_chp_bq_training_pipeline_insert_into_valid_models.cql
