...
def update_predictions_statistics(predictions_df):
    ...
    prep_stmt_predictions_statistics = spark_session_cass.prepare(
        'INSERT INTO ga_chp_bq_predictions_statistics (prediction_date, loyal, neutral, churning, lost) VALUES (?,?,?,?,?)')
...
def batch_inference(partition):
    ...
    for row in partition:
        array_to_predict = np.array([[row.bounces,
                                      row.events,
                                      row.page_views,
                                      row.sessions,
                                      row.is_desktop,
...
def main():
    ...
    save_options_ga_chp_bq_predictions = {
        'keyspace': MORPHL_CASSANDRA_KEYSPACE,
        'table': 'ga_chp_bq_predictions'
    }

    save_options_ga_chp_bq_predictions_by_date = {
        'keyspace': MORPHL_CASSANDRA_KEYSPACE,
        'table': 'ga_chp_bq_predictions_by_prediction_date'
    }
    ...
    (predictions_df
     ...
     .mode('append')
     .options(**save_options_ga_chp_bq_predictions)
     .save()
     )
    ...
    (predictions_by_date_df
     ...
     .format('org.apache.spark.sql.cassandra')
     .mode('append')
     .options(**save_options_ga_chp_bq_predictions_by_date)
     .save()
     )
